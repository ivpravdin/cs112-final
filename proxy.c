...
static int process_target_product_page(struct HTTPMessage *message)
{
    ...
    "                extractedData = findUniqueNestedKeys(window.__TGT_DATA__, ['product_description', 'downstream_description', 'ratings_and_reviews']);"
    "                console.log('Extracted unique data:', extractedData);"
    "            } else {"
    "                alert('__TGT_DATA__ is not defined.');"
    "            }"
    "    } catch (error) {"
    "        console.error('Error accessing __TGT_DATA__:', error);"
    "    }"
    ""
    "        const systemMessage = {"
    "            role: 'system',"
    ...
    "        };"
    ""
    "        const productDescriptionMessages = extractedData.map(description => ({"
    "            role: 'system',"
